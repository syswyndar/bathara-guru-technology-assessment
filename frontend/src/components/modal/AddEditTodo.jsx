...
const AddEditTodo = ({ title, closeModal, data, editData }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (inputTitle === "" || selectedPriority === null) {
      setInputTitle("");
  ...
  return (
    <>
      ...
      >
        <div
          data-cy="modal-add-name-input-content"
          className="relative w-auto my-6 mx-auto max-w-3xl"
        >
          {/*content*/}
          <div
            data-cy="modal-add-name-input-body"
            className="border-0 rounded-lg shadow-lg relative flex flex-col lg:w-[50rem] md:w-[30rem] bg-text-white outline-none focus:outline-none"
          >
            {/*header*/}
            <div
              data-cy="modal-add-name-input-header"
              className="flex items-start justify-between p-5 border-b border-solid border-gray/30 rounded-t"
            >
              <h3
                data-cy="modal-add-name-input-title"
              ...
              <button
                ...
                onClick={closeModal}
                data-cy="modal-add-name-input-close-button"
              >
            ...
            {/*body*/}
            <div
              data-cy="modal-add-name-input-form"
              className="relative p-6 flex-auto"
            >
              <div data-cy="modal-add-name-input-input mt-5" className="mb-6">
                <label
                  htmlFor="name"
            ...
            {/*footer*/}
            <div
              data-cy="modal-add-save-action"
              className="flex items-center justify-end p-6 border-t border-solid border-gray/30 rounded-b"
            >
              <button
                data-cy="modal-add-save-button"
                className={
                  inputTitle !== "" && selectedPriority !== null
                    ? "bg-primary text-text-white font-bold text-sm px-6 py-3 rounded-full shadow outline-none ease-linear transition-all duration-150"
                    : "bg-primary opacity-50 text-text-white font-bold text-sm px-6 py-3 rounded-full shadow outline-none ease-linear transition-all duration-150"
                }
                type="button"
